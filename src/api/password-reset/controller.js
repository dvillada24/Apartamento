...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Apartamentos account.<br>
        ...
        &mdash; Apartamentos Team
      `
      return sendMail({ toEmail: email, subject: 'Apartamentos - Password Reset', content })
    })
